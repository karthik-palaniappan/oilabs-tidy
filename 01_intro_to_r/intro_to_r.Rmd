...
This text is added by **Karthik.**

First, let's load the `present` data frame:

```{r load-present-data-frame}
present
```

Having loaded the `present` data frame, let us now use the `glimpse()` function to examine its dimensions, variables, and ranges for the `year` column.
...
The dimensions of the `present` data frame are as follows:
```{r glimpse-present-view, echo=FALSE,results=TRUE}
glimpse(present)
```

The years included in the data frame range from:
```{r view-year-range,echo=FALSE,results=TRUE}
present %>%
  summarize(min = min(year),
            max = max(year)
  )
...
Compared to Arbuthnot's data set the number of babies baptized in the US in the present time period has gone up by a factor of 200, approximately.
...
ggplot(data=present,aes(x = year, y = boys_ratio, ymin=0.5)) + geom_line()
```

As evident from the graph, the proportion of boys remains over 51% throughout the observation period.
...
```{r sample-arrange, results=TRUE}
present %>%
  arrange(desc(total))
```

From this arrangement, we can see that the total number of births was highest in `1961`.
