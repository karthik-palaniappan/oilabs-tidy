...
So 68 flight meet this criteria.
...
```{r hist-sfo-feb-flights}
ggplot(data = sfo_feb_flights, aes(x = arr_delay)) +
  geom_histogram()
```
```{r sfo-feb-flights-examine-2}
sfo_feb_flights %>%
...
```
Delta and United (DL and UA) have the most variable arrival delays - reason.  highest IQR.
...
Surprisingly, July has the largest departure delays.
...
I'll probably pick the lowest median delay month - this guarantees that in 50% of the cases, my delay will be smaller than the median.  The average could be influenced heavily by outliers.
...
nycflights %>%
  group_by(origin) %>%
  summarise(ot_dep_rate = sum(dep_type == "on time") / n(), n()) %>%
